...
if handles.state.SI.hFastZ.enable
...
try
    ...
    [H,Data,~]=scanimage.util.opentif(fullfile(p,f));
    handles.state=H;
    numPlanes=H.SI.hStackManager.numSlices;
    numChannels=size(Data,3);
    Data(Data==0)=NaN;
    if numPlanes>1
        if numChannels>1
            for i=1:numPlanes;
%                 handles.channels.G{i}=squeeze(Data(:,:,1,i:numPlanes:end));
        ...
        elseif numChannels<2
            for i=1:numPlanes;
%                 handles.channels.G{i}=squeeze(Data(:,:,1,i:numPlanes:end));  % needs further work for simoultaneous R and G channel
                handles.channels.G{i}=squeeze(Data(:,:,1,:,i,i:numPlanes:end));
            end
        end
    else
    ...
    % -- original
%     [H Data]=scim_openTif(fullfile(p,f));
%     handles.state=H;
...
set(handles.zoomFactorBox,'String', ['Zoom: ' num2str(handles.state.SI.hRoiManager.scanZoomFactor)]);
% if ~isnan(handles.state.date)
if ~isfield(handles.state,date)
%     set(handles.DateBox,'String', ['Date: '  handles.state.date]);
    set(handles.DateBox,'String', ['Date: '  '0000']);
end
% set(handles.PowerBox,'String', ['Power: ' num2str(handles.state.beamPowers) '%']);
set(handles.PowerBox,'String', ['Power: ' num2str(handles.state.SI.hBeams.powers(1)) '%']);
% set(handles.FrequencyBox,'String', ['Acq Freq: ' num2str(handles.state.scanFrameRate) ' Hz']);
set(handles.FrequencyBox,'String', ['Acq Freq: ' num2str(handles.state.SI.hRoiManager.scanFrameRate) ' Hz']);
...
%     set(handles.DistanceAcrossPlanesBox,'String',['InterPlane Dist: ', num2str(handles.state.stackZStepSize) ' um']);   
    set(handles.DistanceAcrossPlanesBox,'String',['InterPlane Dist: ', num2str(handles.state.SI.hStackManager.actualStackZStepSize) ' um']);   
else
...
set(handles.NumberOfImagedPlanesBox,'String',['Number of Planes: ' num2str(handles.numberOfPlanes) ]);
set(handles.PlaneNumber,'String',[1:handles.numberOfPlanes],'Value',handles.planeOfInterest);




% handles.timePerLine=handles.state.scanLinePeriod;
handles.timePerLine=handles.state.SI.hRoiManager.linePeriod;
guidata(handles.figure1,handles) ;
...
switch handles.displayChannel
    case 'G'
%         handles.numberOfPlanes=1; %this is a hack gabi april 2021
        if handles.numberOfPlanes>1
            activeChannel=handles.channels.G{handles.planeOfInterest};
        else
            activeChannel=handles.channels.G;
...
for i=startTrialNum:endingTrialNum;
    f = [handles.currentFile(1:end-7) sprintf('%03d',i) '.tif'];
    handles.currentFile=f;
...
assignin('base', ['FAcrossTrials_' handles.currentFile(1:end-7)], FAcrossTrials);
...
%         Leo_image_registration_single_trial_padding_singlePlane(target,fullfile(handles.currentDirectory, handles.currentFile), [saveName 'on_frame_' num2str(currentFrame) '_'],channel,handles.planeOfInterest,handles.numberOfPlanes);
    ...
    case 'Trial Max.'
        ...
        imwrite(uint8(maxProjOfRegisteredTrial), fullfile(handles.currentDirectory, [saveName num2str(currentFrame) '_mean_movAvg.tif']), 'tif');
    case 'Frame'
