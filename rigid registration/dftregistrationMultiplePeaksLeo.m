...
function [row_shift,col_shift] = dftregistrationMultiplePeaksLeo(buf1ft,buf2ft,usfac)
...
elseif usfac == 1,
    ...
    %--------------Leo modifications-----------
%    [xymax,smax,xymin,smin] = extrema2(CC,1);
    numberOfPeaks=5; %numberOfPeaks considered
    [~,maxima] = findExtrema(CC);
   ...
   indices = find(maxima(:));
    ...
    [I J]=ind2sub(size(CC),sortedIndices(1:numberOfPeaks)); % First 5 peaks taken
%     [max1,loc1] = max(CC);
%     [max2,loc2] = max(max1);
%     rloc=loc1(loc2);
%     cloc=loc2;
